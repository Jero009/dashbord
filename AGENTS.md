...
- **Recovery signal**: HRV drives recovery once ≥14 `hrv` rows exist (`HRV_TAKEOVER_MIN`, TrainingLoadOverlay + VitalsPage) — `recoveryBaseline`/`todayRecovery`/overtraining consume `hrv` rows directly. Sleep score deliberately excludes HRV in v1 (would need a `sleep_session` column + migration).
