...
- **SpO₂ readiness input**: `calculateReadinessScore` now includes SpO₂ as an 8-point input (0 pts at ≥3% deviation from baseline, 4 pts at 1.5%, 8 pts at baseline). Gate: ≥14 days of spo2 readings AND stable variance (SD ≤ 2.5). VO₂ max stays display-only — it moves too slowly to score daily. Both inputs are optional: if the gate is inactive or data is missing, the score contributes 0.
