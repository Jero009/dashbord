...
- **Glyph countdown (foreground only)**: while WorkoutPage is open, `tickRestTimer`/`startRestTimer`/`resumeRestTimer`/`adjustRestTimer` push the countdown to the Nothing back display via `showRestGlyph(remaining, total)` (`src/shared/utils/restTimerGlyph.ts`); `clearTimerState` calls `hideRestGlyph()` and `onUnmounted` calls `releaseRestGlyph()`. `buildRestFrame(side, remaining, total)` is pure (unit-tested in `tests/unit/restTimerGlyph.spec.ts`): a depleting border ring (dim) + centred whole-seconds digits (5x7 font for 1–2 digits, 3x5 for 3). App-matrix mode is **foreground-only** — when backgrounded the system reclaims the matrix, so the back display can't update there; the notification chronometer + scheduled ding cover the backgrounded case, and the Glyph re-renders on return to foreground.
